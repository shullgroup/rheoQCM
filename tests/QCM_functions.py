...
def calc_delfstar(n, layers, **kwargs):
    ...
    kwargs:                     
        ...
        reftype (string)
            - 'overlayer' (default):  if overlayer doesn't exists, then the reference  
    ...
    for i in df_in.columns:
        ...
        if not fixed_drho:
            def ftosolve(x):
                ...
                return ([calc_delfstar(n1, layers, calctype=calctype, reftype=reftype).real-
                         df_in[i][n1].real,
                         calc_delfstar(n2, layers, calctype=calctype, reftype=reftype).real-
                         df_in[i][n2].real,
                         calc_delfstar(n3, layers, calctype=calctype, reftype=reftype).imag-
                         df_in[i][n3].imag])
        else:
            def ftosolve(x):
                ...
                if not newtonian:
                    return ([calc_delfstar(n1, layers, calctype=calctype, reftype=reftype).real-
                             df_in[i][n1].real,
                             calc_delfstar(n1, layers, calctype=calctype, reftype=reftype).imag-
                             df_in[i][n1].imag])
        ...
        for n in nplot:
            delfstar_calc[n] = calc_delfstar(n, layers, calctype=calctype, reftype=reftype)

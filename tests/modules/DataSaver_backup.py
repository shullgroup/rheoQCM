...
class DataSaver:
    ...
    def update_mech_df_shape(self, chn_name, nhcalc, refh):
        ...
        mech_keys_single = [
            'drho', # 
            'drho_err', #
            'grho_refh', # h dependent
            'grho_refh_err', # h dependent
            'phi', #
            'phi_err', #
            'dlam_refh', # h dependent
            'lamrho', # h dependent
            'delrho', # h dependent
            'delf_delfsn',
            'rh', # 
        ...
        mech_key = self.get_mech_key(nhcalc, refh)
    ...
    def load_settings(self, path):
        ...
        try: # try to load settings from file
            with h5py.File(path, 'r') as fh:
                ...
                if 'config_default' in fh.keys(): # saved by version < 0.17.0
                    config_default = json.loads(fh['config_default'][()])
    ...
    def update_mech_df_in_prop(self, chn_name, nhcalc, refh, mech_df):
        ...
        getattr(self, chn_name + '_prop')[self.get_mech_key(nhcalc, refh)] = mech_df
        logger.info('mech_df in data_saver') 
        logger.info(getattr(self, chn_name + '_prop')[self.get_mech_key(nhcalc, refh)]) 
        self.saveflg = False
    ...
    def get_mech_df_in_prop(self, chn_name, nhcalc, refh):
        ...
        '''
        return getattr(self, chn_name + '_prop')[self.get_mech_key(nhcalc, refh)].copy()
        ...
        if mode['cryst'] == 'single':
            if mode['temp'] == 'const': # single crystal and constant temperature
                ...
                for seg, ind_list in enumerate(film_idx): # iterate each list
                    ...
                    logger.info('len(fun)', len(self.exp_ref['func'])) 
    ...
    def selector_del_sel(self, chn_name, sel_idx_dict):
        ...
        with h5py.File(self.path, 'a') as fh:
            for harm, idxs in sel_idx_dict.items():
                # delete from raw
                for idx in idxs:
                    logger.info(df_chn.queue_id[idx]) 
                    if 'raw' in fh.keys() and chn_name in fh['raw'] and str(int(df_chn.queue_id[idx])) in fh['raw/'+chn_name] and harm in fh['raw/' + chn_name + '/' + str(int(df_chn.queue_id[idx]))]: # raw data exist
                        logger.info(fh['raw/' + chn_name + '/' + str(int(df_chn.queue_id[idx])) + '/' + harm]) 
    ...
    def import_qcm_with_other_format(self, format, path, config_default, settings=None, f1=None, t0=None, init_file=True):
        '''

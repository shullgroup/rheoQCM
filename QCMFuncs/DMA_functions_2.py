...
def DMAread_sturdy(datadir, file):
    # a read file for Laruen's MAT files
    sample = {}
    data_input = hdf5storage.loadmat(datadir + file)

    freq = np.array(data_input['rawdata'][2,0][:,0])
    estor = np.array(data_input['rawdata'][2,1][:,0])
    eloss = np.array(data_input['rawdata'][2,2][:,0])
    T = np.array(data_input['rawdata'][2,4][:,0])
    estar = np.sqrt(estor**2+eloss**2)
    ...
    dma_np = np.zeros([ftotal, 4])
    dma_np[:, 0] = freq
    dma_np[:, 1] = estar
    dma_np[:, 2] = phi
    dma_np[:, 3] = T
    dma_np = np.reshape(dma_np, (Tnum, fnum, 4))
    dmadata = {}
    dmadata['f'] = dma_np[:, :, 0]
    dmadata['estar'] = dma_np[:, :, 1]
    dmadata['phi'] = dma_np[:, :, 2]    
    dmadata['aT_raw'] = data_input['shift_factors']
...
def DMAplot(sampledata, parms, Tref):
    ...
    if parms.get('aTfit', 'self') == 'self':
        aT = dmadata['aT_raw']/sampledata['aTfix']
    else:
        aT = np.exp(vogel(dmadata['T'], Tref, sp_parms['B'],
                         sp_parms['Tinf']))

    # set up the plots we either re-use current axes or make new ones
    if parms.get('new_axes', 'yes') == 'yes':
        dmafig = plt.figure(sampledata['title'], figsize=(9, 3))
        dma_ax1 = dmafig.add_subplot(1, 3, 1)
        dma_ax1.loglog(True)
        dma_ax1.set_xlabel('$fa_T$ (s$^{-1}$)')
        dma_ax1.set_ylabel('$|E^*|$ (Pa)')
        dma_ax1.set_ylim(bottom=3e5, top=3e9)
        ...
        dma_ax1 = parms['figinfo']['dma_ax1']
        dma_ax2 = parms['figinfo']['dma_ax2']
        dma_ax3 = parms['figinfo']['dma_ax3']
    ...
    for Tindex in Tind:
        ...
        dmadata2[Tindex], = (dma_ax2.plot(faT[Tindex, :],
                             ...
                             linestyle='none'))
        dmadata3[Tindex], = dma_ax3.plot(dmadata['T'][Tindex],
    ...
    #   T_total = samplefits['T_total']
    minT = min(dmadata['T'][Tind])
    maxT = max(dmadata['T'][Tind])
    if 'qcmdata' in sampledata:
        maxT = max(sampledata['qcmdata']['qcm_T'])

    Tforplot = np.linspace(minT, maxT, 100)
    fit3, = dma_ax3.plot(Tforplot, np.exp(vogel(Tforplot, Tref, sp_parms['B'],
                                               sp_parms['Tinf'])), '-b')

    # now add titles to plots
    if parms.get('add_titles', 'yes') == 'yes':
